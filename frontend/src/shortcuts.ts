...
import { isLoggedIn, model } from "./app-context.js";
...
export function handleKeyboardShortcut(event: KeyboardEvent): void {
  ...
  if (event.ctrlKey && event.shiftKey && !event.altKey && !event.metaKey) {
    ...
    if (key === "t") { event.preventDefault(); void ScanActions.startScan("text", model.appState?.settings.lastTextArea || null, true); }
    else if (key === "i" || event.key === "İ" || event.key === "ı") { event.preventDefault(); void ScanActions.startScan("image", model.appState?.settings.lastImageArea || null, true); }
...
export function handleShortcutAction(action: string): void {
  ...
  if (action === "scan-text") { void ScanActions.startScan("text", model.appState?.settings.lastTextArea || null, true); }
  else if (action === "scan-image") { void ScanActions.startScan("image", model.appState?.settings.lastImageArea || null, true); }
